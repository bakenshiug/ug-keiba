"""
🐢 厩舎・直近3週成績スクレイパー
   JRDB joc/tn.php から取得 → docs/data/genbu/trainer/ に保存

使い方:
   export JRDB_USER=xxx
   export JRDB_PASS=yyy
   python3 fetch_trainer_recent.py
"""
import json
import os
import re
...
OUT_DIR = ROOT / "docs" / "data" / "genbu" / "trainer"
OUT_DIR.mkdir(parents=True, exist_ok=True)

URL = "http://www.jrdb.com/member/jrdv/joc/tn.php"
...
def fetch() -> dict:
    ...
    for tr in table.find_all("tr")[1:]:
        ...
        if len(c) < 27:
            continue
        # 列構成: 順/調教師/属/1-5着/着外/勝率/連対率/複勝率/G123/芝123/ダ123/障123/3週前/2週前/1週前
        N = sum(int(c[j] or 0) for j in [3, 4, 5, 6, 7, 8])
        if N < 30:
            continue
        all_data.append({
            "rank":     int(c[0] or 0),
            ...
            "loc":      c[2],
            "N":        N,
            "winRate":  float(c[9]  or 0),
            "showRate": float(c[11] or 0),
            "w3":       parse_pct(c[24]),
            "w2":       parse_pct(c[25]),
            "w1":       parse_pct(c[26]),
        })
...
def main():
    ...
    if "error" in data:
        print(f"❌ {data['error']}")
        sys.exit(1)
    out = OUT_DIR / f"trainer_recent_{data['snapshotDate']}.json"
    with open(out, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    print(f"✅ {data['totalCount']}件保存 → {out}")
    print(f"🔥 HOTリスト ({data['hotList']['count']}件):")
    for x in data["hotList"]["entries"][:10]:
        print(f"   {x['name']:14} {x['loc']} w3={x['w3']:5.1f} w2={x['w2']:5.1f} w1={x['w1']:5.1f}")
